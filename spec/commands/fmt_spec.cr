...
describe Rigor::Commands::Fmt do
  it "emits deterministic stamp yaml" do
    ...
    yaml.lines.first.should eq(%(spec: "0.3"))
  ...
  it "is byte-idempotent on a second fmt run for a mapping-form vouch" do
    path = File.tempname("fmt", ".md")
    File.write(path, "# T\n\n## Stamp\n\n```yaml\nrigor: skimmed\nvouch: {claim: withheld, why: \"fine for scripts, never audited\"}\n```\n")
    Rigor::Commands::Fmt.run(path, IO::Memory.new).should eq(0)
    once = File.read(path)
    once.should contain("Why: fine for scripts, never audited")
    Rigor::Validator.validate(once).valid.should be_true
